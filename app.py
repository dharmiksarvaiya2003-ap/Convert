...
def process_to_bmp(image_pil: Image.Image, w: int, h: int, outline_thick: int, max_fill_pct: float = 40.0):
    ...
    # Dark lines on light bg -> inverse binary (lines = 255)
    _, binary = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
    # Small close to connect gaps
    kernel = np.ones((3,3), np.uint8)
    binary = cv2.morphologyEx(binary, cv2.MORPH_CLOSE, kernel, iterations=1)
    # Direct red-on-white: any design pixel becomes red
    red_arr = np.ones((h, w, 3), dtype=np.uint8) * 255
    red_arr[binary == 255] = [255, 0, 0]
    output = Image.fromarray(red_arr)
    # For outline thickness effect: draw contour lines slightly thicker using binary dilation
    if outline_thick > 1:
        dilated = cv2.dilate(binary, np.ones((outline_thick, outline_thick), np.uint8), iterations=1)
        # Redraw with dilated mask for thicker lines
        red_arr = np.ones((h, w, 3), dtype=np.uint8) * 255
        red_arr[dilated == 255] = [255, 0, 0]
        output = Image.fromarray(red_arr)
    return output, True
...
if process_btn:
    ...
    st.markdown(
        ...
        "<b>Process Complete.</b> Your JPG has been converted to BMP automatically. The figure is filled with <b>Yellow</b>, surrounded by a <b>Red</b> outline of your chosen thickness, on a <b>White</b> ground — exactly as requested for Texcell jacquard work. Sharp curves are preserved via bilateral smoothing and contour averaging.",
        unsafe_allow_html=True,
    )
