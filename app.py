...
def process_to_bmp(image_pil: Image.Image, w: int, h: int, outline_thick: int, max_fill_pct: float = 40.0):
    ...
    # ------------------------------------------------------------------
    # FALLBACK / HYBRID: Direct contours on binary for solid / colored designs
    # (works when design is filled color or outline has gaps)
    # ------------------------------------------------------------------
    direct_contours, _ = cv2.findContours(binary, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    for cnt in direct_contours:
        area = float(cv2.contourArea(cnt))
        # Only keep substantial regions (ignores thin decorative lines that flood-fill already handled)
        if area < min_area * 3:
            continue
        if area > max_area * 2:
            continue
        ...
        if pts.shape[0] < 3:
            continue
        # Gentle smoothing
        window = max(3, min(15, pts.shape[0] // 20))
