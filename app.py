...
def build_pattern(
    ...
    struct = np.ones((max(1, outline_h), max(1, outline_w)), dtype=bool)
    dilated = binary_dilation(mask, structure=struct)
    eroded = binary_erosion(mask, structure=struct)
    outline_ring = dilated & ~eroded
...
with st.sidebar:
    ...
    width_px = st.number_input("Width (pixels)", min_value=10, max_value=2000, value=200, step=1)
    height_px = st.number_input("Height (pixels)", min_value=10, max_value=2000, value=200, step=1)
