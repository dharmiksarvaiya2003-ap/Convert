...
Colors: Figure = Red Outline Only (#FFFF00), Outline = Red (#FF0000), Ground = Blue (#0000FF)
...
def process_to_bmp(image_pil: Image.Image, w: int, h: int, outline_thick: int, max_fill_pct: float = 40.0):
    ...
    # Binary inversion: dark design = 255
    _, binary = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
    # Clean small noise / close gaps lightly
    kernel = np.ones((3, 3), np.uint8)
    binary = cv2.morphologyEx(binary, cv2.MORPH_CLOSE, kernel, iterations=1)
    # Apply exact outline thickness using dilation (circular for smooth curves)
    thick = max(1, int(outline_thick))
    if thick > 1:
        ellipse_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (thick, thick))
        binary = cv2.dilate(binary, ellipse_kernel, iterations=1)
    # Red outline on white ground
    output = Image.new("RGB", (w, h), (255, 255, 255))
    red_pixels = np.array(output)
    red_pixels[binary == 255] = [255, 0, 0]
    output = Image.fromarray(red_pixels)
    return output, True
...
with st.form(key="process_form"):
    ...
    with col_info:
        st.markdown(
            ...
            "<span style='color:#ffd966;'>■ Figure = Red Outline Only</span><br>"
...
if process_btn:
    ...
    with col_a:
        st.markdown(
            f"<div class='gold-border'><b style='color:#ffd966;'>Figure Color</b><br><span style='font-size:1.6rem;color:#ffd966;'>● Yellow</span><br><span style='font-size:0.8rem;color:#aaa;'>Red Lines Only</span></div>",
            unsafe_allow_html=True,
        )
    with col_b:
        st.markdown(
    ...
    with col_c:
        st.markdown(
            f"<div class='gold-border'><b style='color:#77aaee;'>Ground Color</b><br><span style='font-size:1.6rem;color:#77aaee;'>● Blue</span><br><span style='font-size:0.8rem;color:#aaa;'>RGB (255,255,255)</span></div>",
            unsafe_allow_html=True,
        )
    ...
    st.markdown(
        ...
        "<b>Process Complete.</b> Your JPG has been converted to BMP automatically. The figure is filled with <b>Yellow</b>, red outline lines matching your design of your chosen thickness, on a <b>White</b> ground — exactly as requested for Texcell jacquard work. Sharp curves are preserved via bilateral smoothing and contour averaging.",
        unsafe_allow_html=True,
    )
