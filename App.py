...
# --- 2. Advanced Vector Tracing (Exact Pixel Control) ---
def process_jacquard_exact_pixels(input_image, img_w, img_h, reed, pick, out_w, out_h, threshold, mode):
    try:
        ...
        # Step 2: First Skeletonization (Original size)
        try:
        ...
        # Step 4: Create Blank Target Canvas and Draw
        ...
        # Step 5: THE FIX - Second Skeletonization on Final Canvas
        # This guarantees absolutely NO double pixels after scaling
        try:
            canvas = cv2.ximgproc.thinning(canvas, thinningType=cv2.ximgproc.THINNING_GUOHALL)
        except:
            pass # Failsafe
            
        # Step 6: THE FIX - Exact Mathematical Pixel Shifting
        # Completely replaces dilation brush to guarantee strict WxH dimensions
        out_w = max(1, int(out_w))
        out_h = max(1, int(out_h))
        
        final_mask = canvas.copy()
        
        # Apply exact Width (shift right by exactly out_w - 1 pixels)
        if out_w > 1:
            for i in range(1, out_w):
                shifted = np.zeros_like(canvas)
                shifted[:, i:] = canvas[:, :-i]
                final_mask = cv2.bitwise_or(final_mask, shifted)
                
        # Apply exact Height (shift down by exactly out_h - 1 pixels)
        if out_h > 1:
            temp_mask = final_mask.copy()
            for i in range(1, out_h):
                shifted = np.zeros_like(temp_mask)
                shifted[i:, :] = temp_mask[:-i, :]
                final_mask = cv2.bitwise_or(final_mask, shifted)
...
if check_password():
    ...
    with col_title:
        ...
        st.write("Guaranteed 100% Exact Pixel Thickness (No double pixels)")
    with col_logout:
        if st.button("Logout"):
            st.session_state["password_correct"] = False
            st.rerun()
    ...
    if uploaded_file is not None:
        ...
        st.markdown("---")
        st.subheader("⚙️ 6 Formatting Options (Exact Match System)")
        ...
        if st.button("🚀 Generate Perfect BMP"):
            with st.spinner(f"Creating strictly {out_w}x{out_h} pixel lines..."):
                ...
                if isinstance(result, str) and result.startswith("Error"):
                    ...
                    st.success(f"✅ Perfect BMP Generated! (હવે પહોળાઈ {out_w} અને ઊંચાઈ {out_h} એક્ઝેટ બનશે!)")
                    st.download_button(
                        label="📥 Download BMP",
                        data=result,
                        file_name="jacquard_flawless.bmp",
                        mime="image/bmp"
                    )
